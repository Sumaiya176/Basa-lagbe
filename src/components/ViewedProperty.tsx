...
const ViewedProperty = () => {
  const { data } = useGetViewedPropertyQuery(undefined);
  const [removeViewedProperty] = useRemoveViewedPropertyMutation();
  const [currentPage, setCurrentPage] = useState<number>(1);
  const [itemsPerPage] = useState<number>(5);

  console.log("data recently", data?.data);
  ...
  const handleRemoveProperty = async (id: string) => {
    try {
      const result = await removeViewedProperty(id).unwrap();
      console.log(result);
      if (result?.isSuccess) {
  ...
  return (
    <div className="p-4">
      <p className="text-3xl font-extrabold mb-10">Recently Viewed Listings</p>
